...
M.send_line_or_selection = function()
  ...
  if vim.bo.filetype == "r" or vim.bo.filetype == "quarto" then
    if M.is_piped_expression() then
      ...
      local text = vim.api.nvim_buf_get_lines(0, range[1] - 1, range[2], false)
      text = table.concat(text, "\n")
      vim.fn['slime#send'](text .. "\n")
      return
    end
...
M.new_terminal_python = function() M.new_terminal('python') end
M.new_terminal_ipython = function() M.new_terminal('ipython --no-confirm-exit') end
M.new_terminal_r = function() M.new_terminal('R') end
M.new_terminal_julia = function() M.new_terminal('julia') end
M.new_terminal_shell = function() M.new_terminal('$SHELL') end
